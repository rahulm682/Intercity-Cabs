...
import { Paper, Box, Typography, Stack, Chip, MenuItem, Select, FormControl } from '@mui/material';
...
interface Props {
  inquiry: InquiryData;
  onStatusChange: (id: string, newStatus: string) => void; // Parent handles the logic
}
  ...
  return (
    <Paper 
      ...
      sx={{ 
        ...
        borderColor: `${statusColors[inquiry.status]}.main`, // Dynamic Border Color
        transition: '0.2s',
        '&:hover': { boxShadow: 4 }
      }}
    >
      ...
      {inquiry.message && (
        <Box bgcolor="#f9fafb" p={1.5} borderRadius={2} border="1px solid #eee">
          <Typography variant="body2" fontStyle="italic" color="text.primary">"{inquiry.message}"</Typography>
        </Box>
      )}
    </Paper>
  );
